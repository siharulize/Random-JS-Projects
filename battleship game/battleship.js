...
var model = {
	...
	generateShip: function(){
		...
		var col;
		if (direction === 1){
			//location for horizontal ship
			row = Math.floor(Math.random() * this.boardSize);
			col = Math.floor(Math.random() * (this.boardSize - (this.shipLength + 1 )));
		} else {
			//location for vertical ship
	...
	processGuess: function(guess){
		...
		if (location){
			...
			if (hit && model.shipsSunk === model.numShips) {
				view.displayMessage("You sank all my battleships, in " + this.guesses + " guesses. " + "To continue please reload the game.");
